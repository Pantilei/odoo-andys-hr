...
class EmployeeChart(http.Controller):
    ...
    def get_employee_ids(self):
        ...
        if len(employees) == 1:
            ...
            raise UserError(
                _("These employee have no Manager %s. For correct construction of chart only one single emloyee at the top of chart(Director of company) must not contain any manager!") % (names))
    ...
    def get_parent_child(self, **post):
        if post:
            ...
            table = """<table><tr><td colspan='""" + \
                str(len(child_ids) * 2) + """'><div class="node">"""
